...
public interface UsuarioRepositorio extends JpaRepository<Usuario, Integer> {
    ...
    @Query("select u.idUsuario from Usuario u where u.eliminado = false and concat(u.nombre, concat(' ', u.apellido)) like concat('%', concat(?1,'%') ) and u.correo like concat('%',concat(?2,'%')) and u.rol.idRol in (?3)")
    List<Integer> findAllByFilters(String nombre, String correo, List<Integer> idRoles);
